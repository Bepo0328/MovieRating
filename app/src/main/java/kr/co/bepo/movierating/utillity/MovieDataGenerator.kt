...
class MovieDataGenerator {
    ...
    fun generate() {
        ...
        movieData.forEach {
            firestore.collection(DBKey.DB_COLLECTION).add(it)
        }
    }
    ...
    private val movieData = listOf(
        ...
        Movie(
            isFeatured = false,
            title = "블랙 위도우",
            actors = "스칼렛 요한슨 , 플로렌스 퓨 , 레이첼 와이즈",
            country = "미국",
            director = "케이트 쇼트랜드",
            genre = "#액션 #어드벤처 #SF",
            posterUrl = "http://file.koreafilm.or.kr/thm/02/99/17/33/tn_DPF022685.jpg",
            rating = "12세 관람가",
            averageScore = 4.4f,
            numberOfScore = 1_365_983,
            releaseYear = 2021,
            runtime = 134
        )
    )
